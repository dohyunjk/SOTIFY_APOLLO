...
bool PathTimeHeuristicOptimizer::SearchPathTimeGraph(
  ...
  // DEBUG DOHYUN
  ADEBUG << "ST DRIVABLE BOUNDARY by dohyun";
  double tmp = 0;
  double v_inst = 0;
  for (const auto& instance : reference_line_info_->st_graph_data().st_drivable_boundary().st_boundary()) {
    v_inst = (instance.s_upper() - tmp) / 0.1;
    ADEBUG << "T: " << instance.t();
    ADEBUG << "S_LOWER: " << instance.s_lower();
    ADEBUG << "S_UPPER: " << instance.s_upper();
    ADEBUG << "V_OBS_LOWER: " << instance.v_obs_lower();
    ADEBUG << "V_OBS_UPPER: " << instance.v_obs_upper();
    ADEBUG << "V_INST: " << v_inst;
    tmp = instance.s_upper();
  }

...
Status PiecewiseJerkSpeedNonlinearOptimizer::Process(
  ...
  if (!problem_setups_status.ok()) {
    speed_data->clear();
    ADEBUG << "QP problem setups status false by dohyun";
  ...
  if (!qp_smooth_status.ok()) {
    speed_data->clear();
    ADEBUG << "QP smooth status false by dohyun";
    return qp_smooth_status;
  }
  ////////DEBUG
  //ADEBUG << speed_data->DebugString() << " by dohyun";
  ADEBUG << "QP result by dohyun";
  ADEBUG << "Distance: " << distance[0] << " by dohyun";
  ADEBUG << "velocity: " << velocity[0] << " by dohyun";
  ADEBUG << "acceler: " << acceleration[0] << " by dohyun";
  ////////////
  ...
  if (speed_limit_check_status) {
    ...
    if (!path_curvature_smooth_status.ok()) {
      speed_data->clear();
      ADEBUG << "QP path curvature smooth status false by dohyun";
    ...
    if (!speed_limit_smooth_status.ok()) {
      speed_data->clear();
      ADEBUG << "QP speed limit smooth status false by dohyun";
    ...
    if (!nlp_smooth_status.ok()) {
      speed_data->clear();
      ADEBUG << "QP nlp smooth status false by dohyun";
...
Status PiecewiseJerkSpeedNonlinearOptimizer::SetUpStatesAndBounds(
  ...
  ADEBUG << "piecewise init val: " << s_dot_init_ << ", " << s_ddot_init_ << " by dohyun";

  // Set s_dot bounary
  s_dot_max_ = std::fmax(FLAGS_planning_upper_speed_limit,
                         st_graph_data.init_point().v());
  //ADEBUG << "upper speed limit: " << FLAGS_planning_upper_speed_limit << " by dohyun";
...
Status PiecewiseJerkSpeedNonlinearOptimizer::OptimizeByQP(
  ...
  ADEBUG << "dx bounds: " << std::fmax(FLAGS_planning_upper_speed_limit, init_states[1]) << " by dohyun";

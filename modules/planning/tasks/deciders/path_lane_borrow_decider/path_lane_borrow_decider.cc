...
Status PathLaneBorrowDecider::Process(
  ...
  if (!blocking_obstacle_id.empty()) {
      ...
      if (blocking_obstacle != nullptr) {
          ...
          AINFO << "Distance to blocking obstacle = "
              << blocking_obstacle_s - reference_line_info->AdcSlBoundary().end_s() << " by dohyun";
      }
  }
/*
  ...
  if (reference_line_info->GetBlockingObstacle() != nullptr) {
    ...
    double blocking_obstacle_s =
              reference_line_info->GetBlockingObstacle()->PerceptionSLBoundary().start_s();

    AINFO << "Distance to blocking obstacle = "
              << blocking_obstacle_s - reference_line_info->AdcSlBoundary().end_s() << " by dohyun";
  }
*/
  AINFO << "Ego vehicle speed: "
        << frame->PlanningStartPoint().v() << " by dohyun";
  // Debugging part end (dohyun)
  ...
  if (Decider::config_.path_lane_borrow_decider_config()
    ...
    AINFO << "In lane borrow scenario?: " << reference_line_info->is_path_lane_borrow() << " by dohyun";
  }
  return Status::OK();
}
...
bool PathLaneBorrowDecider::IsNecessaryToBorrowLane(
  ...
  if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {
    ...
    if (mutable_path_decider_status->able_to_use_self_lane_counter() >= 6) { //(Dohyun) counter 값이 6 이상이면 차선 변경이 아니라 기존 차선을 그대로 사용
    ...
    AINFO << " LANE BORROW SPEED CHECK by dohyun";
    
    if (!IsWithinSidePassingSpeedADC(frame)) {
      return false;
    }
    AINFO << " LANE BORROW SPEED CHECK passed by dohyun";
    ...
    AINFO << " OBSTACLE CHECK passed by dohyun";

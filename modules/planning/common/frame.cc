...
const Obstacle *Frame::FindCollisionObstacle(const EgoInfo *ego_info) const {
  ...
  AINFO << "vehicle heading: " << ego_info->vehicle_state().heading() << " by dohyun";
  ...
  for (const auto &obstacle : obstacles_.Items()) {
    ...
    int i = 0;
    for (const auto &ob_point : obstacle_polygon.points()) {
      AINFO << "Obstacle points: " << i
            << " x: " << std::fmod(ob_point.x(), 100.f)
            << " y: " << std::fmod(ob_point.y(), 100.f) << " by dohyun";
      i++;
    }
    if (obstacle_polygon.HasOverlap(adc_polygon)) {
      for (const auto &adc_point : adc_polygon.points()) {
        AINFO << "ADC points: " << i
              << " x: " << std::fmod(adc_point.x(), 100.f)
              << " y: " << std::fmod(adc_point.y(), 100.f) << " by dohyun";
        i++;
      }
      return obstacle;
    }

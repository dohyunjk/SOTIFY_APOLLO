...
void OnLanePlanning::RunOnce(const LocalView& local_view,
  ...
  // DOHYUN DEBUG
  std::list<ReferenceLine> tmp_reference_lines;
  std::list<hdmap::RouteSegments> tmp_segments;
  reference_line_provider_->GetReferenceLines(&tmp_reference_lines, &tmp_segments);
  for (const auto& tmp : tmp_reference_lines) {
    ADEBUG << "reference line length: " << tmp.Length();
  }
...
Status OnLanePlanning::Plan(
  ...
  // Public road planner run DOHYUN

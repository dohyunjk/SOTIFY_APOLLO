...
Status LaneFollowStage::PlanOnReferenceLine(
  ...
  //////////// OBSTACLE DECISION DEBUGGING by dohyun
  for (const auto* obs_item_ptr : reference_line_info->path_decision()->obstacles().Items()) {
    // Sanity checks.
    Obstacle* obs_ptr = reference_line_info->path_decision()->Find(obs_item_ptr->Id());
    if (obs_ptr == nullptr) {
      const std::string msg = "Null obstacle pointer.";
      AERROR << msg;
    }

    //ADEBUG << "longitudinal decision: " << obs_ptr->LongitudinalDecision() << " by dohyun";
    ADEBUG << "longitidual decision: " << obs_ptr->DebugString() << " by dohyun";
  }
  ...
  ADEBUG << reference_line_info->reference_line().Length();
  //ADEBUG << reference_line_info->path_data().discretized_path().Length();
  //////////////////// DEBUG END //////////////////////////

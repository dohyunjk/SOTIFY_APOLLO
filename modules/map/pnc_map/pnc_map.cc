...
double PncMap::LookForwardDistance(double velocity) {
  ...
  auto forward_distance = velocity * (FLAGS_look_forward_time_sec + 2) + 50;
...
bool PncMap::GetRouteSegments(const VehicleState &vehicle_state,
  ...
  // DEBUG DOHYUN
  ADEBUG << "Vehicle velocity: " << vehicle_state.linear_velocity();
  ADEBUG << "LookForwardDistance: " << look_forward_distance;
